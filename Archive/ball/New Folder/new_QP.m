...
for k = 1:1
    ...
    % ----------------------------------
    z_predicted         = psi*xbar_k + gamma*ubar_km1 + theta*dU_k + ...
                            phi*f0d + repmat(g0, Hp, 1);
    xbar_predicted      = A_big*xbar_k + AB_big*ubar_km1 + ABdu_big*dU_k + ...
...
%     plot(linspace(0, Hp*Ts, Hp+1), [z0(1); z_predicted(1:p:end)], 'k-');
    plot(linspace(0, Hp*Ts, Hp+1), [x0(1); x_predicted(1:n:end)], 'k-');
    subplot(2,1,2);
