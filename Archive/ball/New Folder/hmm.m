...
%     ubar_km1    = zeros(size(u0));
    ...
    % ----------------------------------
    % Prediction matrices
    epsilon = T_fancy - psi*x0 - gamma*u0;
    ...
    Omega   = [F_fancy; 
               ...
               W];
    omega   = [-F_1*u0 - f;
               -Tau*(psi*x0 + gamma*u0) - g;
               -w];
           
    % Solve Quadratic Program
    % ----------------------------------
    [dU, fval, ~, ~, ~] = quadprog(2*H, -G, Omega, omega);
    ...
    u_previous              = u0;
    for h = 1:Hp
        delta_u                 = dU(m*(h-1)+1:h*m,:);
        U(m*(h-1)+1:h*m, 1)     = u_previous + delta_u;
        u_previous              = U(m*(h-1)+1:h*m, 1);   
    end
    U_k     = U;
    uk      = U_k(1:m,:);
    ...
    % ----------------------------------
    y_predicted  = psi*x0 + gamma*u0 + theta*dU;
    x_predicted  = A_big*x0 + AB_big*u0 + ABdu_big*dU;
